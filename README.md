...
- [Echan1208/DeepSeek-Harness-Setup](https://github.com/Echan1208/DeepSeek-Harness-Setup) — DeepSeek Harness Windows offline installer bundling all dependencies for network-free setup, with an office-productivity boost via Univer + ppt-master.
- [fan56/dsh-cron](https://github.com/fan56/dsh-cron) — Cron scheduling for the DeepSeek Harness — bounded tasks with calendar & interval rules, delivered to live agents.
- [fan56/dsh-model-sync](https://github.com/fan56/dsh-model-sync) — A dsh (DeepSeek Harness) Cordis plugin that keeps llm-pi-ai provider routes' model catalog in sync with the pi.dev gateway — written through the official settings seam, zero patches to dsh internals.
- [idoall/dsh-update-status](https://github.com/idoall/dsh-update-status) — Read-only DeepSeek Harness Web version status and release-channel guidance plugin.
- [Me-Maped/dsh-ask](https://github.com/Me-Maped/dsh-ask) — Direct terminal Q&A mode for DeepSeek Harness: persistent sessions, streaming output, and optional Fish/Bash/Zsh integration plus experimental PowerShell support.
...
- [yyh-001/dsh-launcher](https://github.com/yyh-001/dsh-launcher) — Lightweight Windows launcher for DeepSeek Harness: pick a version, launch dsh web.
...
- [AnLifeX/dsh-attention](https://github.com/AnLifeX/dsh-attention) — Reminder plugin for DeepSeek Harness: don't miss the moments that need you.
- [NinjaSln-labs/dsh-context-compass](https://github.com/NinjaSln-labs/dsh-context-compass) — Session-health plugin for DeepSeek Harness: a real-data "continue vs. start fresh" context compass — header badge, /compass, the context_compass tool, and a multi-session overview panel.
- [ohmejj/dsh-chat-archive](https://github.com/ohmejj/dsh-chat-archive) — Automatically archives DeepSeek Harness conversation logs.
...
- [Zoria-Lind/dsh-token-optimizer](https://github.com/Zoria-Lind/dsh-token-optimizer) — Layered token-optimization pipeline for DSH: long-text→image summarization with ask-first dialog (token-economics estimate), dynamic-resolution tiers, disk summary cache + batched vision summarization, output-ladder compression, repeated-read diff, tool-trim + lazy MCP load, compaction driver, session stats with cache-hit rate.
- [Zoria-Lind/dsh-behavior-enhancer](https://github.com/Zoria-Lind/dsh-behavior-enhancer) — Companion behavior-management plugin: tool-call discipline prompt, failure-triggered parallelism convergence (drop to 1, auto-restore), consecutive-failure user intervention, post-write syntax check with .bak snapshot rollback. Complements dsh-token-optimizer.
...
- [OMSociety/kimi-ppt-skill](https://github.com/OMSociety/kimi-ppt-skill) — DSH (DeepSeek Harness) presentation skill: PPT/deck generation and export, with a pure-local export path (python-pptx) and local preview rendering (Pillow) inside DSH.
...
- [hanrr92/dsh-code-quote](https://github.com/hanrr92/dsh-code-quote) — Code quote / citation plugin for DeepSeek Harness (dsh).
- [Shyboy0499/dsh-pr-watch](https://github.com/Shyboy0499/dsh-pr-watch) — DeepSeek Harness plugin: reports what changed in your authored pull requests since your last check — merged, closed, or gone stale.
- [TheHeartFickle/dsh-conversation-folding](https://github.com/TheHeartFickle/dsh-conversation-folding) — DSH conversation-stream rendering enhancement: folds the process, keeps the substance — cleaner long conversations.
...
- [thomasvvugt/dsh-kanban-flow](https://github.com/thomasvvugt/dsh-kanban-flow) — Agent-driven kanban board for DeepSeek Harness: one board per workspace, per-task agent sessions, guarded human/agent workflow.
- [weixshaw/dsh-plugin-task-runner](https://github.com/weixshaw/dsh-plugin-task-runner) — Task Runner decompose-and-dispatch agent preset for DeepSeek Harness: the main agent breaks a task down, subagents (default local model) run it in parallel, then results are synthesized; concurrency/model configurable per machine.
- [Xplore-LAB/dsh-plugin-office](https://github.com/Xplore-LAB/dsh-plugin-office) — Office-document plugin for DeepSeek Harness: DOCX/XLSX/CSV handling and mail-merge productivity tools (agent-tools, cordis-plugin).
...
- [batteryshark/orchestra-next](https://github.com/batteryshark/orchestra-next) — Durable agent runs for local fleets, built on the DeepSeek Harness (dsh). Streamlined Orchestra rewrite: one execution path, stdlib Python, SQLite, three-file console.
- [Erick114514/dsh-agent-hub](https://github.com/Erick114514/dsh-agent-hub) — DeepSeek Harness host plugin: Agent Hub mailbox tools shared by deepseek / claude-code / codex.
...
- [onclaw-dev/dsh-workflow-designer](https://github.com/onclaw-dev/dsh-workflow-designer) — Prompt-first workflow designer plugin for DeepSeek Harness: collects the Agent's visible local Skills, native Tools, and MCP Tools, offers visual orchestration and constraint editing, and exports a stable YAML intermediate representation that models/frameworks can later turn into Python, `workflow.mjs`, or other code workflows.
...
- [vecnode/dsh-vn-plugins](https://github.com/vecnode/dsh-vn-plugins) — Personal DeepSeek Harness plugin pack for Windows, providing a docked file tree and a CodeMirror text editor for CLI web and desktop profiles.
- [weilantianhai/dsh-command-skill-list](https://github.com/weilantianhai/dsh-command-skill-list) — Slash-command skill picker for DeepSeek Harness, with concise Chinese/English description translation, cached summaries, and custom notes.
...
- [prgrmrwy/dsh-cockpit](https://github.com/prgrmrwy/dsh-cockpit) — Multi-device DeepSeek Harness cockpit: manage DSH on many machines from one page, jump straight into each device's native workbench (SSH loopback tunnel + read-only status aggregation, zero changes on the remote).
- [softspark/dsh-process-console](https://github.com/softspark/dsh-process-console) — Processes tab for DeepSeek Harness: every agent and subagent of a conversation as a live console with paired tool requests and responses, installed as one profile bundle.
...
- [See-Sol-Lab/DeepSeekGUI](https://github.com/See-Sol-Lab/DeepSeekGUI) — Windows desktop workbench that bundles the official DSH runtime into a one-click installer and keeps it fully isolated from any existing dsh install — installing it breaks nothing. Changes / Git / Worktrees / Memory views beside the conversation, approval-gated Git/PR tools, user-editable global memory plus assistant-maintained project memory; follows official DSH releases. Experimental Linux AppImage.
...
- [statem-li/dsh-think-tools](https://github.com/statem-li/dsh-think-tools) — DSH thinking & tool-call aggregation UI: turn-level thinking chip (live duration/scrolling text) + tool-call aggregation chip + shared activity drawer + conversation-flow cards (step/summary cards, appearing only after the turn ends) — ported from dsh-webui. Zero DSH core modification.
- [wenyifeng2026qq/dsh-client-ui-glass](https://github.com/wenyifeng2026qq/dsh-client-ui-glass) — Toggleable glassmorphism theme skin for the DeepSeek Harness web client.
- [White-Egret/DSH-Desktop](https://github.com/White-Egret/DSH-Desktop) — A modern Windows desktop application for DSH built with Tauri v2, Rust, and Vanilla JS.
...
- [keeshakulbida948-tech/dsh-plugin-sticker](https://github.com/keeshakulbida948-tech/dsh-plugin-sticker) — Emoji-sticker plugin for DeepSeek Harness: the AI searches and sends stickers from a built-in 109-image whale-girl meme pack, rendered inline in the chat bubble by a custom client toolview (no file downloads); pairs with dsh-plugin-fun.
...
- [lucky8197/dsh-devquest](https://github.com/lucky8197/dsh-devquest) — Turns development into an RPG for DSH: XP from turns/tools/todos, 47 achievements with rarity tiers, daily/weekly challenges, daily lottery, seasonal shop, 7 theme skins, multiple titles, an activity calendar, and a hall of honor. Event-stream driven — auto-scores once installed.
...
- [statem-li/dsh-chat-flow](https://github.com/statem-li/dsh-chat-flow) — DSH turn-level thinking chip (live duration/scrolling text) + tool-call aggregation chip + shared activity drawer + conversation-flow cards (step/summary cards, appearing only after the turn ends) — ported from dsh-webui. Zero DSH core modification.
...
- [oxlyn/dsh-flyout-sidebar](https://github.com/oxlyn/dsh-flyout-sidebar) — Flyout sidebar plugin for DeepSeek Harness.
- [simune/dsh-desktop](https://github.com/simune/dsh-desktop) — Plugin management workspace plus a Tauri-based desktop client (dsh-desktop) for DeepSeek Harness, bundling the dsh Web service into a native window to simplify desktop deployment and usage.
- [weibaohui/dsh-file-share](https://github.com/weibaohui/dsh-file-share) — DSH plugin · directory sharing: browse/upload/download any configurable directory over HTTP, plus @-mention a file from the composer for the agent to process.
- [flykit-cc/dsh-flykit](https://github.com/flykit-cc/dsh-flykit) — Explorer with live file watch, agent terminals (Claude Code, Pi, Codex), DSH agent tools, a searchable model picker and a Claude-Code-style status line.
- [having5548/deepseek-harness-desktop](https://github.com/having5548/deepseek-harness-desktop) — A WinUI-based desktop client for DeepSeek Harness.
- [hpyperry/dsh-cajita](https://github.com/hpyperry/dsh-cajita) — Grab-bag of small DeepSeek Harness Web plugins: an inline edit button for sent messages, a web_fetch policy toggle to allow private-network targets, and turn-completion/approval browser notifications.
- [Small-tailqwq/dsh-internet-angel-desktop](https://github.com/Small-tailqwq/dsh-internet-angel-desktop) — Unofficial pixel desktop skin for DSH Web inspired by NEEDY GIRL OVERDOSE. Experimental, non-commercial fan project.
- [weibaohui/dsh-xiuxian](https://github.com/weibaohui/dsh-xiuxian) — Immortal-cultivation companion plugin: randomly wakes a character from Mortal's Journey to Immortality as a chibi floating pet while you code (ships with a 2496-character dataset).
- [xi-kari/dsh-amphoreus](https://github.com/xi-kari/dsh-amphoreus) — Amphoreus × DSH: a Golden Seat workspace and canvas workbench built on DeepSeek Harness (unofficial).
...
- [AQian0/dsh-desktop](https://github.com/AQian0/dsh-desktop) — A simple Tauri-based desktop wrapper for dsh.
...
- [Elave-66/dsh-blue-sea-launcher](https://github.com/Elave-66/dsh-blue-sea-launcher) — Anime/Galgame-style launcher icon for DeepSeek Harness, featuring a whale-girl character.
- [moxingovo/dsh-web-panel](https://github.com/moxingovo/dsh-web-panel) — Embed the DeepSeek Harness (dsh) web GUI in VS Code — zero-config auto-start, workspace follow, self-healing.
- [Wanbinyu/dsh-launcher](https://github.com/Wanbinyu/dsh-launcher) — Double-click Windows launcher and tray app for DeepSeek Harness Web.
- [Binaryinject/dsh-tray](https://github.com/Binaryinject/dsh-tray) — Native, dependency-free system tray launcher for DeepSeek Harness.
- [daha1216/dsh-font-customizer](https://github.com/daha1216/dsh-font-customizer) — Font customization plugin for DeepSeek Harness: change UI/code fonts and body text size, detect locally installed fonts, and persist settings.
...
- [edgeseeker7/dsh-subagent-progress](https://github.com/edgeseeker7/dsh-subagent-progress) — DSH subagent progress dock showing child-session activity and reported updates above the conversation composer.
- [jaibhasin/dsh-browser-agent](https://github.com/jaibhasin/dsh-browser-agent) — Chrome side-panel client and DSH bridge plugin for browser snapshots, navigation and interaction over a local WebSocket connection; requires building both components.
...
- [shenmituzi/img-share](https://github.com/shenmituzi/img-share) — DSH skill: share browser screenshots / local images in chat (thumbnail + fullscreen, phone-visible).
